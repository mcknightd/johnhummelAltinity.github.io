...
4. On each server fill out file cluster.xml in /etc/clickhouse-server/config.d/ folder with cluster, shard, and replica definitions.  This is the main cluster for distributed tables.\
\
...
5. Add additional cluster definitions to cluster.xml
...
6. Test the cluster to ensure definitions are correct.  Create the following table definitions.  \
 \
Do not name a distributed table the same as the database, it leads to error 'Distributed table test looks at itself''.

    ```sql
    CREATE TABLE test_local ON CLUSTER '{cluster}' (id UInt8, ts DateTime)
        Engine=ReplicatedMergeTree('/clickhouse/tables/{database}/{shard}/{table}', '{replica}')
        ORDER BY (id)
        PARTITION BY toYYYYMM(ts)
        TTL ts + INTERVAL 7 MONTH TO DISK 'hdd_1'
...
7. Insert data into the table(s) and ensure it replicates as expected.
...
**NOTE: Cleaning up Zookeeper data manually can corrupt replication if you make a mistake.  Raise a support ticket and ask for help if you have *any* doubt concerning the procedure.**
...
2. Login to the server host and check local connectivity using clickhouse-client. If you cannot connect, stop here and address the problem.
    ...
    4. If an HTTP port, use curl, e.g., `curl http://localhost:8123/ping` or `curl https://localhost:8123/ping`.
3. If applicable connect to ClickHouse from the remote host that is having problems.
    ...
    6. If an HTTP port, use curl, e.g., `curl http://host_or_ip_address:8123/ping` or `curl https://host_or_ip_address:8123/ping`.
...
1. Replicate the problem from the command line.
    ...
    2. For clients that use the HTTP interfaces, try submitting a request using curl with basic authentication.  Example: `curl --user myuser:secret http://localhost:8123/?query=select%201`.
...
1. Are merges working properly?.
    1. Try to merge manually using the steps below.
        ...
        1. Try to merge manually in ClickHouse to find out why it is not working as shown in the following example. (‘2019-10-07’ is the part name.)
